...
namespace gps {
	...
	void Model3D::ReadOBJ(std::string fileName, std::string basePath) {
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
					...
					all.push_back(currentVertex.Position);
		...
		for (glm::vec3 i : all) {
			this->center.x += i.x / all.size();
			this->center.y += i.y / all.size();
			this->center.z += i.z / all.size();
		}
	...
	void Model3D::setCenter(glm::vec3 center) {
		this->center = center;
	}
